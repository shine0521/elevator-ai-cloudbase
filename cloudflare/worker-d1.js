/**
 * 特种设备电梯AI系统 - Cloudflare Worker (D1 Session 版)
 * 使用 D1 存储 Session，无需 KV
 */
...
async function handleDashboard(session, env) {
  ...
  return html(DASHBOARD_HTML
    .replaceAll('{{user.name}}', session.name)
    .replaceAll('{{user.role}}', session.role)
    .replaceAll('{{todayDiscriminations}}', todayCount?.c || 0)
    .replaceAll('{{pendingAudits}}', pendingCount?.c || 0)
    .replaceAll('{{activeTemplates}}', activeTemplates?.c || 0)
    .replaceAll('{{compliantRate}}', compliantRate)
    .replaceAll('{{recentRecords}}', recent?.results?.map(r => `
    ...
    .replaceAll('{{chartData}}', JSON.stringify(chartData.results || []))
  );
}
...
async function handleLogin(request, env) {
  ...
  if (!email || !password) {
    return html(LOGIN_HTML.replaceAll('{{error}}', '请输入邮箱和密码'));
  }
  ...
  if (!user) {
    return html(LOGIN_HTML.replaceAll('{{error}}', '用户不存在'));
  }
  
  const passwordHash = await hashPassword(password);
  if (passwordHash !== user.password_hash) {
    return html(LOGIN_HTML.replaceAll('{{error}}', '密码错误'));
  }
...
async function handleAiClassify(request, session, env) {
  ...
  const rules = await env.DB
    .prepare("SELECT * FROM rule_engine_rules").all();
  ...
  const result = matched ? (matched.action_value || '待人工') : '待人工';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/api/health') {
      return json({ status: 'ok', env: 'cloudflare-workers', version: '4.0.0-d1' });
    }
    ...
    if (path === '/logout') {
      const cookies = parseCookies(request.headers.get('Cookie') || '');
      if (cookies.ev3_tok) {
        await deleteSession(cookies.ev3_tok, env);
      }
      return new Response(null, {
        status: 302,
...
async function handleLogs(session, env) {
  ...
  return html(LOG_HTML.replaceAll('{{logs}}', logs.results?.map(l => `
...
async function handleTemplates(session, env) {
  ...
  return html(TEMPLATE_HTML.replaceAll('{{templates}}', templates.results?.map(t => `
...
async function handleKnowledge(session, env) {
  ...
  return html(KNOWLEDGE_HTML.replaceAll('{{regulations}}', regs.results?.map(r => `
    <div class="reg-card">
      <h4>${r.code} - ${r.name}</h4>
...
async function handleAudit(session, env) {
  ...
  return html(AUDIT_HTML.replaceAll('{{tasks}}', tasks.results?.map(t => `
...
async function handleSettings(session, env) {
  return html(SETTINGS_HTML.replaceAll('{{user.name}}', session.name).replaceAll('{{user.email}}', session.email));
}
...
async function handleHistory(session, env) {
  return html(HISTORY_HTML);
}
